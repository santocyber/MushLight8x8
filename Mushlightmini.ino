...
int maxNumberFilesSaveInSPIFFS = 0; //SPIFFS is about 1300KB and each picture is 100KB, so can only store about 10 photos
...
//  randomSeed(analogRead(0) + analogRead(1));
 ...
 initSPIFFS();
  ...
  logger = readFile (SPIFFS, FILE_PHOTO);
...
//    request->send_P(200, "text/plain", "Taking Photo");
  ...
  //  request->send(SD_MMC, path.c_str(), "image/jpg", false);
  });

   // Route to set GPIO state to HIGH
        server.on("/fotos", HTTP_GET, [](AsyncWebServerRequest *request) {
          //ledState = "";
//      request->send(SPIFFS, "/index.html", "text/html", false, processor);
